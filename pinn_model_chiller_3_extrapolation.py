...
class CustomLoss(tf.keras.losses.Loss):
    def call(self, y_true, y_pred):
        ...
        evaporator_inlet_temp = tf.cast(X_scaled[:, 1], tf.float32)
        evaporator_outlet_temp = tf.cast(X_scaled[:, 2], tf.float32)
        evaporator_flow_rate = tf.cast(X_scaled[:, 3], tf.float32)
        condenser_refrigerant_pressure = tf.cast(X_scaled[:, 4], tf.float32)
        condenser_inlet_temp = tf.cast(X_scaled[:, 5], tf.float32)
        condenser_outlet_temp = tf.cast(X_scaled[:, 6], tf.float32)
        condenser_flow_rate = tf.cast(X_scaled[:, -1], tf.float32)

        # Physics-based equation for motor load prediction
        ...
        total_loss = physics_loss + 15 * mse_loss
        return total_loss
...
threshold_extrap = 6.177517703933928
...
results_extrap_df.to_csv('extrapolated_vs_actual_motor_kw.csv', index=False)

# Save extrapolation outliers to CSV
outliers_extrap_df = pd.DataFrame({
    'Outlier Difference': outliers_extrap
})
outliers_extrap_df.to_csv('outliers_motor_kw_extrapolation.csv', index=False)
