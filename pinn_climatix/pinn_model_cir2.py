...
def custom_loss(y_true, y_pred):
    ...
    eev2_open_valve = tf.cast(X_train[:, 0], tf.float32)
    ...
    1 / (
        ...
        (
            ...
            (
                ...
                (
                    ((eev2_open_valve / 1.768) - 1.486) - condenser_pressure_probe
                )
            )
        )
    )
    ...
    mse_scaled = 20*(mse / (K.mean(K.abs(y_true)) + epsilon))
    eqn_diff_scaled = 0.1*(eqn_diff / (K.mean(K.abs(eqn)) + epsilon))
